import{t as e}from"./supabaseClient-D86BU6sx.js";import{t}from"./usuariosService-Bz2WJqyy.js";import{t as n}from"./constants-iuLjF5j6.js";var r=`https://generativelanguage.googleapis.com/v1beta/models/gemini-3.5-flash-lite:generateContent`,i=`
...
`,a=async(e,t)=>{if(!t)throw Error(`No API Key provided`);let n=e.map(e=>({role:e.role===`ai`?`model`:`user`,parts:[{text:e.text}]})),a={systemInstruction:{parts:[{text:i}]},contents:n,generationConfig:{responseMimeType:`application/json`,temperature:0}};try{let e=await fetch(`${r}?key=${t}`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify(a)});if(!e.ok){let t=await e.json();throw e.status===429||t.error?.code===429?Error(`Límite de peticiones de IA alcanzado (Error 429). Espera unos 50 segundos antes de intentar nuevamente.`):e.status===503||t.error?.code===503?Error(`El modelo de IA está muy saturado ahora mismo (Error 503). Intenta de nuevo en unos momentos.`):Error(t.error?.message||`Error al conectar con Gemini`)}let n=(await e.json()).candidates[0].content.parts[0].text,i=n.indexOf(`{`),o=n.lastIndexOf(`}`);return i!==-1&&o!==-1&&(n=n.substring(i,o+1)),JSON.parse(n)}catch(e){throw console.error(`Gemini API Error:`,e),e}},o=e=>{let t=[],n={},r={},i=[];for(let t of e){let{action_type:e,student_id:a,group_id:o}=t,s=o||`[FALTA_ID_GRUPO]`,c=a||`[FALTA_ID_ESTUDIANTE]`;e===`enroll_user`?(n[s]||(n[s]=new Set),n[s].add(c)):e===`remove_user`?(r[s]||(r[s]=new Set),r[s].add(c)):i.push(t)}for(let[e,n]of Object.entries(r)){let r=Array.from(n).map(e=>`"${e}"`).join(`,`);t.push(`magik run:prod pull:user:from:group["${e}",${r}]`)}for(let[e,r]of Object.entries(n)){let n=Array.from(r).map(e=>`"${e}"`).join(`,`);t.push(`magik run:prod enroll:user["${e}",${n}]`)}for(let e of i){let{action_type:n,student_id:r,program_id:i,group_id:a,status_id:o}=e,s=i||`[FALTA_ID_PROGRAMA]`,c=a||`[FALTA_ID_GRUPO]`,l=r||`[FALTA_ID_ESTUDIANTE]`;switch(n){case`change_status`:let r=o||`"${e.status_name}"`;t.push(`magik run:prod status:change["${s}","${r}","${l}"]`);break;case`undo_publication`:t.push(`magik run:prod undo:publication ["${c}"]`);break;case`recalculate_grades`:t.push(`magik run:prod:force final:user ["${c}","${l}"]`);break;case`audit_statistics`:t.push(`magik run:prod audit:statistics["${s}","${l}"]`),a&&t.push(`magik run:prod audit:subject ["${a}","${l}"]`),t.push(`magik run:prod audit:compacts["${s}","${l}"]`);break;case`clean_cache_sislms`:t.push(`magik run:prod cache:clean:sislms ["*"]`);break;case`clean_cache_crm`:t.push(`magik run:prod cache:clean:crm ["*"]`);break;case`fix_deliverable`:t.push(`magik run:prod attempts:fix ["${c}","${l}"]`);break;default:console.warn(`Action type no reconocido: ${n}`)}}return t};function s(e){let t=(e.replace(/\b[a-f0-9]{24}\b/gi,`___OID___`).match(/\b\d{3,7}\b/g)||[]).filter(e=>{let t=parseInt(e);return!(t>=2020&&t<=2030)});return[...new Set(t)]}function c(e){let t=e.match(/\b[a-f0-9]{24}\b/gi)||[];return[...new Set(t)]}async function l(t,n){if(!t.length)return{};try{let{data:r}=await e.from(`programas`).select(`mongo_id, name`).eq(`alliance_id`,n).in(`mongo_id`,t);return r?Object.fromEntries(r.map(e=>[e.mongo_id,e.name])):{}}catch(e){return console.error(`Error resolviendo nombres de programas:`,e),{}}}async function u(e,r,i){let a=n[r],o=s(e),u=c(e);if(o.length===0)return{students:[],objectIds:u,enrichedContext:``};i&&i(`resolving_students`);let f=[],p=[];for(let e of o)try{let n=await t(e,a);if(n&&n._id&&n._id.$oid){let t=(n.programs||[]).map(e=>{let t=e.structure?.$oid||e.structure;return t&&p.push(t),{id:t}}).filter(e=>e.id);f.push({inc:e,objectId:n._id.$oid,name:n.profile?.full_name||`Sin nombre`,email:n.profile?.email||``,programs:t,programCount:t.length,autoProgram:t.length===1?t[0]:null})}}catch(t){console.warn(`No se pudo hidratar INC ${e}:`,t.message)}if(p.length>0){let e=await l([...new Set(p)],a);for(let t of f){for(let n of t.programs)n.name=e[n.id]||n.id;t.autoProgram&&(t.autoProgram.name=e[t.autoProgram.id]||t.autoProgram.id)}}return{students:f,objectIds:u,enrichedContext:d(f,u)}}function d(e,t){if(e.length===0)return``;let n=`
...
USA ESTOS DATOS DIRECTAMENTE EN TUS ACCIONES. No preguntes por student_id ni program_id si ya están aquí.`,n}var f=class{constructor(e,t){this.apiKey=e,this.alliance=t}normalizeStr(e){return e.normalize(`NFD`).replace(/[\u0300-\u036f]/g,``).toLowerCase()}async preprocessMessage(r,i,a){let o=r,s=``,c=a||r;try{let e=await u(c,this.alliance,i);e.enrichedContext&&(s+=e.enrichedContext)}catch(e){console.error(`Error en hydration de estudiantes:`,e)}let l=r.match(/https:\/\/sis\.kuepa\.com\/students\/details\/([a-f0-9]{24})\?structure_id=([a-f0-9]{24})/i);if(l){let r=l[1],i=l[2];try{let a=await t(r,n[this.alliance]),o=`desconocido`;if(a){let{data:t}=await e.from(`programas`).select(`mongo_id, name`).eq(`alliance_id`,n[this.alliance]).eq(`mongo_id`,i).single();t&&(o=t.name)}s+=`\n[CONTEXTO DE URL: El usuario proporcionó una URL de SIS. student_id: "${r}", program_id: "${i}" (Programa: "${o}"). USA ESTOS IDs DIRECTAMENTE.]`}catch(e){console.error(`Error en RAG de URL:`,e),s+=`\n[CONTEXTO DE URL: student_id: "${r}", program_id: "${i}".]`}}return`${o}${s}`}async resolveMagicVariables(r,i){let a={...r},o=null;if(a.student_id)try{if(o=await t(a.student_id,n[this.alliance]),o&&o._id&&o._id.$oid)a.student_id=o._id.$oid;else if(a.student_id.length<24&&/^\d+$/.test(a.student_id))throw Error(`INCOMPLETE:El estudiante con INC ${a.student_id} no fue encontrado en tu base de datos.`)}catch(e){if(e.message.startsWith(`INCOMPLETE`))throw e;console.error(`Error resolviendo usuario:`,e)}if(!a.program_id&&(a.action_type===`audit_statistics`||a.action_type===`change_status`)){if(o&&o.programs&&o.programs.length>0){if(o.programs.length===1)a.program_id=o.programs[0].structure?.$oid||o.programs[0].structure;else throw Error(`INCOMPLETE:El estudiante tiene múltiples programas registrados (${o.programs.length}). Por favor, proporciona el ID del programa específico al que te refieres.`)}else throw Error(`INCOMPLETE:No se encontró ningún programa asociado al estudiante. ¿Tienes el ID del programa académico?`)}if(a.action_type===`change_status`&&a.status_name&&!a.status_id){let{data:t}=await e.from(`estados`).select(`mongo_id, name`).eq(`alliance_id`,n[this.alliance]),r=(t||[]).find(e=>this.normalizeStr(e.name)===this.normalizeStr(a.status_name));if(r)a.status_id=r.mongo_id;else throw Error(`INCOMPLETE:El estado "${a.status_name}" no existe en la alianza actual. Por favor verifica el nombre.`)}return a}async processMessage(e,t,n){if(!this.apiKey)throw Error(`No API Key provided`);let r=t.filter(e=>e.role===`user`&&!e.isHidden).map(e=>e.text).join(`
`),i=r?`${r}\n${e}`:e,s=await this.preprocessMessage(e,n,i);n(`ai`);let c=t.map(e=>{let t=e.text;return e.role===`ai`&&!t&&e.parsedResult?.type===`COMMANDS`&&(t=`[Acciones generadas y ejecutadas por el sistema. No repetir.]`),{role:e.role,text:t}}),l=t.length>0?`[CONTINUACIÓN]`:`[NUEVA TAREA]`;c.push({role:`user`,text:`${l}: ${s}`});try{let e=await a(c,this.apiKey);if(e&&(e.type===`INCOMPLETE`||e.type===`INFO`||e.type===`QUERY`||e.type===`ROUTE`))return e;if(e&&e.type===`ACTIONS`&&e.actions){n(`db_processing`);let r=[];for(let n of e.actions){let e=await this.resolveMagicVariables(n,t);r.push(e)}let i=o(r);return i.filter(e=>e.includes(`[FALTA_`)).length>0?{type:`INCOMPLETE`,message:`No se pudieron resolver todos los datos necesarios para generar los comandos. Verifica que los IDs proporcionados sean correctos.`}:{type:`COMMANDS`,commands:i}}if(e&&e.type===`COMMANDS`)return e;throw Error(`Respuesta no soportada del agente.`)}catch(e){if(e.message&&e.message.startsWith(`INCOMPLETE:`))return{type:`INCOMPLETE`,message:e.message.replace(`INCOMPLETE:`,``)};throw e}finally{n(null)}}};export{f as AgentOrchestrator};